...
@synthesize linkedInLogoutDegate;
//
// OAuth step 1a:
//
...
//    [request setParameters:params];
    OARequestParameter * scopeParameter=[OARequestParameter requestParameter:@"scope" value:@"r_basicprofile+r_fullprofile+r_emailaddress+rw_nus"];
...
- (void)requestTokenResult:(OAServiceTicket *)ticket didFinish:(NSData *)data
{
    ...
    if([responseBody rangeOfString:@"timestamp_refused"].location != NSNotFound)
    {
        [[[UIAlertView alloc]initWithTitle:@"" message:@"Cannot login to Linkedin please synchronize date and time in settings" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil]show];
...
{
    apikey = LINKED_IN_API_KEY;
    secretkey = LINKED_IN_API_SECERET_KEY;
...
+(OAConsumer*)getConsumer
{
    ...
    NSString* key = LINKED_IN_API_KEY;
    NSString* secret = LINKED_IN_API_SECERET_KEY;
    return  [[OAConsumer alloc]initWithKey:key secret:secret realm:@"http://api.linkedin.com/"];
}


-(void)logoutLinkedInWithDelegate:(id<LinkedInLoginDelegate>)aLinkedInLoginDelegate andAccessToken:(OAToken*)token
{
    self.linkedInLogoutDegate = aLinkedInLoginDelegate;
    
    NSURL* logoutUrl = [NSURL URLWithString:@"https://api.linkedin.com/uas/oauth/invalidateToken"];
    OAMutableURLRequest *request =
    [[[OAMutableURLRequest alloc] initWithURL:logoutUrl
    ...
    [fetcher fetchDataWithRequest:request
                         delegate:self
                didFinishSelector:@selector(logoutResult:didFinish:)
                  didFailSelector:@selector(logoutResult:didFail:)];
}

- (void)logoutResult:(OAServiceTicket *)ticket didFinish:(NSData *)data
{
    NSString *responseBody = [[NSString alloc] initWithData:data
                                                   encoding:NSUTF8StringEncoding];
    NSLog(@"%@",responseBody);
    if([self.linkedInLogoutDegate respondsToSelector:@selector(linkedInLogOutSuccess)]){
        
    
        [self.linkedInLogoutDegate linkedInLogOutSuccess];
    }
}

- (void)logoutResult:(OAServiceTicket *)ticket didFail:(NSData *)error 
{
    NSLog(@"%@",[error description]);
    
    if([self.linkedInLogoutDegate respondsToSelector:@selector(linkedInLogOutFailed)]){
        
        
        [self.linkedInLogoutDegate linkedInLogOutFailed];
    }
}
